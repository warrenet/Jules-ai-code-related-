...
The easiest way is to use the installer script. It will create a single directory `~/termux-ai-toolkit` and place all the necessary files inside.

Just copy the entire `install_toolkit.sh` script and paste it into your Termux terminal.
...
The toolkit is designed with a simple, modular architecture:

*   **`ai_cli.sh` (The Core):** This is the central script that directly communicates with the AI providers (OpenAI and Gemini). It handles argument parsing, API key management, and the actual API calls. All other functional scripts are wrappers around this core script.

*   **Wrapper Scripts (`clip_summarize.sh`, `url_summarize.sh`, `file_summarize.sh`):** These scripts provide user-friendly ways to perform common tasks. They are responsible for gathering the input data (from the clipboard, a URL, or a file), preparing it, and then passing it to `ai_cli.sh` with a specific system prompt tailored for summarization. Some of these scripts, like `file_summarize.sh` and `url_summarize.sh`, include a "map-reduce" logic to handle large inputs by breaking them into smaller chunks.

*   **Configuration and Setup:**
    *   `install_toolkit.sh`: A self-extracting installer that creates the toolkit directory and unpacks all the scripts.
    *   `01_env_template.sh`: A template for users to create their own `~/.config/termux-ai/env` file to store API keys and other settings.
    *   `00_check_env.sh`: A diagnostic tool to verify that all dependencies are installed and API keys are set.

*   **Testing:**
    *   `tests.sh`: A set of smoke tests to ensure the basic functionality of the scripts.

This modular design makes the toolkit easy to understand, maintain, and extend.
...
*   **Example 2: Summarize a project README**
    ```bash
    bash file_summarize.sh -f '~/termux-ai-toolkit/README.md'
    ```
